...
@Component
@Slf4j
public class BinConfigHierarchyLevelLkpListener {
	@Autowired
	private SyncTableService syncTableService;

	@KafkaListener(topics = "${app.340b_db_host}.dbo.bin_config_hierarchy_level_lkp")
	public void listen(@Payload(required = false) String message) throws Exception {
		if (message != null) {

			String operation = "";
			BinConfigHierarchyLevelLkpDto dto = new BinConfigHierarchyLevelLkpDto();
			try {
				log.info("Message Received for bin_config_hierarchy_level_lkp Table");
				...
				Gson gson = new Gson();
				dto = gson.fromJson(jsonStr, BinConfigHierarchyLevelLkpDto.class);
				syncTableService.binConfigHierarchyLevel(dto, operation);
