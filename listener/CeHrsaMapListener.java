...
@Component
@Slf4j
public class CeHrsaMapListener {

	
	@Autowired
	private SyncTableService syncTableService;

	@KafkaListener(topics = "ce_hrsa_map")
	public void listen(@Payload(required = false) String message) throws Exception {
		if (message != null) {

			String operation = "";
			CeHrsaMapDto dto = new CeHrsaMapDto();
			try {
				log.info("Message Received for ce_hrsa_map Table");
				...
				Gson gson = new Gson();
				dto = gson.fromJson(jsonStr, CeHrsaMapDto.class);
				syncTableService.ceHrsaMap(dto, operation);
